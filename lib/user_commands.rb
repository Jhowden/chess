class UserCommands
  ...
  VALID_EN_PASSANT_EXPRESSION = /^[a-h]{1}[1-8]{1}\s?[a-h]{1}[1-8]{1}\s?e.p.$/
  ...
  def user_move_input
    ...
    if VALID_EXPRESSIONS_COLLECTION.any? { |expression| input =~ expression }
      ...
      puts "Please enter a correctly formated move (ex: b3 b6, 0-0 to castle kingside, 0-0-0 to castle queenside), or b4 c3 e.p. to perform en_passant:"
      user_move_input
    end
  end
end