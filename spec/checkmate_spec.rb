...
describe Checkmate do
  ...
  before(:each) do
    ...
    allow( king ).to receive( :position ).and_return king
    allow( king ).to receive( :file ).and_return "b"
    allow( king ).to receive( :rank ).and_return 4
    allow( game ).to receive( :board ).and_return board
    allow( game ).to receive( :replace_board )
    allow( piece ).to receive( :replace_board )
    allow( piece2 ).to receive( :replace_board )
    allow( piece3 ).to receive( :replace_board )
    ...
    context "when king has possible moves" do
      it "checks to see where a king can move and not be in check" do
        ...
        allow( game ).to receive( :convert_to_file_and_rank ).
          and_return( [[2, 4], [2, 5], [2, 6], [3, 4], [3, 6], [4, 4], [4,5], [4, 6]] )
        allow( king ).to receive( :rank ).and_return 3
